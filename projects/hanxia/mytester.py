...
def parse_args():
    ...
    parser.add_argument('maxtests', type=int, default=-1, help='Maximum #actions to test')
...
def main():
    ...
    while(time.time() < start + config.timeout) and (actioncount < config.maxtests):
        for s in states:
            i += 1
            ...
            for w in xrange(0, config.width):
                for d in xrange(0, config.depth):
                    if (time.time() > start + config.timeout) or (actioncount >= config.maxtests):
                        break
                    ok = check_action()
