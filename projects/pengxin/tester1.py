...
def main():
    ...
    while time.time()-start < BUDGET:
        sut.restart()
        if (random.random() > 0.4) and (test != None):
            print "Exploiting.."
            sut.backtrack(test)
        storedTest = False
        print "Testing.."
        ...
        print "Weight the coverage.."
        for j in sortedCov:
            weight = (defCover - coverageCount[j])
            weightedCov = j*weight
            if weightedCov > coverage4:
                coverage2.append(weightedCov)
                print "Coverage:", j
                print "Backtracking...."
